use std::{fs, collections::{HashMap, LinkedList, HashSet}, env};
...
fn main() {
    ...
    generate_storage_and_simplify(ll, forb, no_labels, max_signal,  prime_field, apply_non_linear_simplification);
}
...
pub fn generate_storage_and_simplify(constraints: LinkedList<Constraint<usize>>, forb: HashSet<usize>, no_labels: usize, max_signal: usize,  field: BigInt, apply_simp: bool){
    ...
    let signalmap = constraint_list::constraint_simplification::simplification(linear, & mut storage, forb, no_labels, max_signal,  field.clone(), apply_simp);
    let cl = constraint_list::r1cs_porting::ConstraintList{
        field : field,
        constraints : storage,
        no_public_inputs : no_labels,
        no_public_outputs : no_labels,
        no_private_inputs : no_labels,
        no_labels : no_labels,
        signal_map : signalmap
    };   
    println!("Ahora comienza la pega");
